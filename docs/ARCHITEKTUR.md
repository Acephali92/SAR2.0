...
**Astro-Anbindung:** `src/lib/payload-loader.ts` fetcht bei gesetztem `PAYLOAD_URL` live von Payloads REST-API (nur `freigabeStatus: veroeffentlicht`, `publishedAt <= jetzt`) — Nichterreichbarkeit lässt den Build laut fehlschlagen. Unabhängig vom Filter des Loaders erzwingt die API selbst für anonyme und API-Key-Zugriffe `freigabeStatus = veroeffentlicht` **und** `_status = published` (`cms/src/access/readPublishedOrSession.ts`) — der Build kann also auch mit geänderter Query keine Entwürfe erhalten. Bilder lädt der Loader mit dem API-Key herunter, weil die Mediathek anonym gesperrt ist. Ohne `PAYLOAD_URL` (lokal ohne laufendes `cms/`, und in CI) wird stattdessen `src/content/_fixtures/{beitraege,termine}.json` gelesen, damit `npm run verify` auch ohne Docker/Postgres lauffähig bleibt.

**Redaktionelle Vorschau — Grenze zur öffentlichen Seite:** Beiträge und Termine haben in der Admin-Oberfläche einen Vorschau-Knopf (`collection.admin.preview`), der auf eine eigene Admin-Route `redaktion.stoppramstein.de/admin/vorschau/:collection/:id` zeigt (`cms/src/components/VorschauView.tsx`, registriert über `admin.components.views`). Diese Ansicht liest das Dokument per Local API **inklusive Entwurfsstand** (`draft: true`) und rendert es in einem schlichten Artikel-/Event-Layout, das Typografie und Abstände der öffentlichen Seite grob nachbildet.

Die Grenze ist strikt: stoppramstein.de bleibt `output: 'static'` und sieht Entwürfe **nie**. Es gibt keine öffentliche Vorschau-URL, keinen Astro-Adapter, keinen Server-Mode, kein Preview-Cookie und keinen Entwurfs-Pfad in `payload-loader.ts` — der Loader fragt weiterhin ausschließlich `freigabeStatus: veroeffentlicht` ab. Die Vorschau existiert vollständig innerhalb von `cms/` und verlangt eine Anmeldung.

> ⚠️ **Sicherheitsrelevant, im installierten Payload-Stand (3.89) verifiziert:** Payload hängt Custom-Admin-Views **nicht** automatisch hinter den Login-Redirect — `isCustomAdminView` in `@payloadcms/next` nimmt jede unter `admin.components.views` registrierte Route ausdrücklich von der Auth-Prüfung aus. `VorschauView.tsx` prüft `req.user` deshalb selbst und zeigt ohne Session die Nicht-gefunden-Seite statt des Entwurfsinhalts; der Aufruf eines fremden Dokuments mit der Rolle Autor endet in einem echten HTTP 404. Wer künftig weitere Custom-Views ergänzt, muss diese Prüfung ebenfalls selbst mitbringen.

**Versionsrechte:** `versions.drafts` läuft auf beiden CMS-Collections. Wer welche Version lesen und zurückholen darf, regeln `cms/src/access/canReadVersions.ts` (gebunden an `access.readVersions`) und der `beforeChange`-Hook `cms/src/hooks/restrictVersionRestore.ts`. Der Hook ist nötig, weil Payload kein eigenes Zugriffsrecht fürs Wiederherstellen kennt: die `restoreVersion`-Operation prüft nur `access.update`. Details siehe [`cms/README.md`](../cms/README.md).
